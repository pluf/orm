...
class ExpressionTest extends PlufTestCase
{
    ...
    {
        // pass as string
        $this->assertEquals('now()', $this->e('now()')
            ->render());
        // pass as array without key
        $this->assertEquals('now()', $this->e([
            'now()'
        ])
            ->render());
        ...
        $this->assertEquals('now()', $this->e([
            ...
            ->render());
        // pass as array without key
        $this->assertEquals(':a Name', $this->e([
            '[] Name'
        ], [
            'First'
        ])
    ...
    {
        // argument = Expression
        $this->assertEquals('hello, world', $this->e('hello, [who]', [
            'who' => $this->e('world')
        ])
            ->render());

        // multiple arguments = Expression
        $this->assertEquals('hello, world', $this->e('[what], [who]', [
            'what' => $this->e('hello'),
            'who' => $this->e('world')
        ])
            ->render());
        ...
        $this->assertEquals('testing "hello, world"', $this->e('testing "[]"', [
            $this->e('[what], [who]', [
                'what' => $this->e('hello'),
                'who' => $this->e('world')
            ])
    ...
    public function testNestedParams()
    {
        ...
        $this->assertEquals('++1 and --2', strip_tags($e1->getDebugQuery()));

        $e2 = $this->e('=== [foo] ===', [
            'foo' => $e1
        ]);

        $this->assertEquals('=== ++1 and --2 ===', strip_tags($e2->getDebugQuery()));
    ...
    public function testNestedExpressions()
    {
        ...
        $e2 = $this->e('[greeting]! How are you.', [
            'greeting' => $e1
        ]);
        $e3 = $this->e('It is me again. [greeting]', [
            'greeting' => $e1
        ]);
    ...
    public function testEscape()
    {
        ...
        $this->assertEquals('"first_name"', $this->callProtected($this->e(), '_escape', [
            'first_name'
        ]));
        $this->assertEquals('"123"', $this->callProtected($this->e(), '_escape', [
            123
        ]));
        $this->assertEquals('"he""llo"', $this->callProtected($this->e(), '_escape', [
            'he"llo'
        ]));

        // should not escape expressions
        $this->assertEquals('*', $this->callProtected($this->e(), '_escapeSoft', [
            '*'
        ]));
        $this->assertEquals('"*"', $this->callProtected($this->e(), '_escape', [
            '*'
        ]));
        $this->assertEquals('(2+2) age', $this->callProtected($this->e(), '_escapeSoft', [
            '(2+2) age'
        ]));
        $this->assertEquals('"(2+2) age"', $this->callProtected($this->e(), '_escape', [
            '(2+2) age'
        ]));
        $this->assertEquals('"users"."first_name"', $this->callProtected($this->e(), '_escapeSoft', [
            'users.first_name'
        ]));
        $this->assertEquals('"users".*', $this->callProtected($this->e(), '_escapeSoft', [
            'users.*'
        ]));
        $this->assertEquals(true, $this->callProtected($this->e(), '_escapeSoft', [
            new \stdClass()
        ]) instanceof \stdClass);
        ...
        $this->assertEquals('"first_name"', $this->e()
            ->escape('first_name')
            ->render());
        $this->assertEquals('"first""_name"', $this->e()
            ->escape('first"_name')
            ->render());
        $this->assertEquals('"first""_name {}"', $this->e()
            ->escape('first"_name {}')
    ...
    public function testConsume()
    {
        ...
        $this->assertEquals('"123"', $this->callProtected($this->e(), '_consume', [
            123,
            'escape'
        ]));
        $this->assertEquals(':x', $this->callProtected($this->e([
            '_paramBase' => 'x'
        ]), '_consume', [
            123,
            'param'
        ...
        $this->assertEquals('(select *)', $this->callProtected($this->e(), '_consume', [
            new Query()
        ]));

        $this->assertEquals('hello, "myfield"', $this->e('hello, []', [
            new MyField()
        ])
            ->render());
    }
    ...
    {
        $this->expectException(\Exception::class);
        $this->callProtected($this->e(), '_consume', [
            123,
            'blahblah'
        ]);
    }
    ...
    {
        $e = new JsonExpression('hello, [who]', [
            'who' => 'world'
        ]);
