...
class Query extends Expression
{
    ...
    /**
     * Query will use one of the predefined templates.
     * The $mode will contain
     ...
     *            Should we add aliases, see _render_field_noalias()
     *            
    ...
    protected function _render_field($add_alias = true)
    {
        ...
        foreach ($this->args['field'] as $alias => $field) {
            ...
            // - alias is numeric
            if ($add_alias === false || (is_string($field) && $alias === $field) || is_numeric($alias)) {
                $alias = null;
            }
     ...
     *
     * @param bool $add_alias
     *            Should we add aliases, see _render_table_noalias()
     *            
     * @return string Parsed template chunk
     */
    ...
    protected function _render_with()
    {
        ...
        $isRecursive = false;
        foreach ($this->args['with'] as $alias => [
            'cursor' => $cursor,
            'fields' => $fields,
            'recursive' => $recursive
        ]) {
    ...
    public function _render_join()
    {
        ...
        foreach ($this->args['join'] as $j) {
            ...
            if (isset($j['expr'])) {
                ...
                $jj .= $this->_escape($j['fa'] ?: $j['f1']) . '.' . $this->_escape($j['f2']) . ' = ' . ($j['m1'] === null ? '' : $this->_escape($j['m1']) . '.') . $this->_escape($j['m2']);
            }
            $joins[] = $jj;
        }
     ...
     *
     * @param mixed $field
     *            Field, array for OR or Expression
     * @param mixed $cond
     *            Condition such as '=', '>' or 'is not'
     * @param mixed $value
     *            Value. Will be quoted unless you pass expression
     * @param string $kind
     *            Do not use directly. Use having()
     * @param string $num_args
    ...
    public function _render_limit()
    {
        if (isset($this->args['limit'])) {
            return ' limit ' . (int) $this->args['limit']['shift'] . ', ' . (int) $this->args['limit']['cnt'];
        }
    }
    ...
    /**
     * Orders results by field or Expression.
     ...
     * If the template is not explicitly set will use "select" mode.
     *
     * @return string
     */
    ...
    /**
     * Switch template for this query.
     * Determines what would be done
     * on execute.
     *
     ...
     * sub-class (e.g.
     * Query_MySQL will return Expression_MySQL).
     *
