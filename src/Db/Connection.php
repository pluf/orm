...
class Connection
{
    ...
    public static function normalizeDSN($dsn, $user = null, $pass = null)
    {
        ...
        if ($parts !== false && isset($parts['host'], $parts['path'])) {
            ...
            $dsn = $parts['scheme'] . ':host=' . $parts['host'] . (isset($parts['port']) ? ';port=' . $parts['port'] : '') . ';dbname=' . substr($parts['path'], 1);
     ...
     * @return Connection
     */
    public static function connect($dsn, $user = null, $password = null, $args = []): Connection
    {
        ...
        if ($dsn instanceof \PDO) {
            ...
            switch ($driver) {
                case 'pgsql':
                    $connectionClass = Connection\PgSQL::class;
                    $queryClass = Query\PgSQL::class;
                    break;
                case 'oci':
                    $connectionClass = Connection\Oracle::class;
                    break;
                case 'sqlite':
                    $queryClass = Query\SQLite::class;
                    break;
                case 'mysql':
                    $expressionClass = Expression\MySQL::class;
                default:
                    // Default, for backwards compatibility
                    $queryClass = Query\MySQL::class;
                    break;
            }
        ...
        if (is_object($dsn)) {
            return new Connection\Proxy(array_merge([
                'connection' => $dsn
            ], $args));
        }
        ...
        switch ($dsn['driver']) {
            case 'mysql':
                $c = new static(array_merge([
                    ...
                    'expression_class' => Expression\MySQL::class,
                    'query_class' => Query\MySQL::class,
                    'driver' => $dsn['driver']
                ], $args));
                break;
            ...
            case 'sqlite':
                $c = new static(array_merge([
                    'connection' => new \PDO($dsn['dsn'], $dsn['user'], $dsn['pass']),
                    'query_class' => Query\SQLite::class,
                    'driver' => $dsn['driver']
                ], $args));
                break;

            case 'oci':
                $c = new Connection\Oracle(array_merge([
                    'connection' => new \PDO($dsn['dsn'], $dsn['user'], $dsn['pass']),
                    'driver' => $dsn['driver']
                ], $args));
                break;
            ...
            case 'oci12':
                ...
                $c = new Connection\Oracle12(array_merge([
                    'connection' => new \PDO($dsn['dsn'], $dsn['user'], $dsn['pass']),
                    'driver' => $dsn['driver']
                ], $args));
                break;
            ...
            case 'pgsql':
                $c = new Connection\PgSQL(array_merge([
                    'connection' => new \PDO($dsn['dsn'], $dsn['user'], $dsn['pass']),
                    'driver' => $dsn['driver']
                ], $args));
                break;
            ...
            case 'dumper':
                $c = new Connection\Dumper(array_merge([
                    'connection' => static::connect($dsn['rest'], $dsn['user'], $dsn['pass'])
                ], $args));
                break;

            case 'counter':
                $c = new Connection\Counter(array_merge([
                    'connection' => static::connect($dsn['rest'], $dsn['user'], $dsn['pass'])
                ], $args));
                break;
     ...
     * @return Query
     */
    public function query($properties = []): Query
    {
        $c = $this->query_class;
        $q = new $c($properties);
        $q->connection = $this;
    ...
    public function beginTransaction()
    {
        ...
        $r = $this->inTransaction() ? false : $this->connection->beginTransaction();
