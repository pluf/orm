 ...
 * Optionally specify a string - a piece
...
class Expression implements \ArrayAccess, \IteratorAggregate, ResultSet
{
    ...
    /**
     * As per PDO, _param() will convert value into :a, :b, :c ..
     * :aa .. etc.
     *
     * @var string
     */
     ...
     *
     * @param
     *            string The offset to assign the value to
     * @param
     ...
     *
     * @param mixed $sql_code
     *            Expression
     * @param string $escape_mode
     *            Fall-back escaping mode - param|escape|none
     *            
    ...
    {
        return is_object($value) || $value === '*' || strpos($value, '(') !== false || strpos($value, $this->escape_char) !== false;
    }

    /**
     * Soft-escaping SQL identifier.
     * This method will attempt to put
    ...
    /**
     * Creates new expression where $sql_code appears escaped.
     * Use this
    ...
    /**
     * Converts value into parameter and returns reference.
     * Use only during
     * query rendering. Consider using `_consume()` instead, which will
     * also handle nested expressions properly.
     *
     * @param string|array $value
     *            String literal or array of strings containing input data
     *            
    ...
    public function render()
    {
        ...
        $res = preg_replace_callback(
            ...
            '/\[[a-z0-9_]*\]|{[a-z0-9_]*}|{{[a-z0-9_]*}}/i', function ($matches) use (&$nameless_count) {
